...
def notify(msg: str):
    try:
        subprocess.run(
            ["python3", str(SCRIPTS_DIR / "notify.py"), msg],
...
def watch(transcript_path: str, session_id: str):
...
def cmd_start():
    ...
    proc = subprocess.Popen(
        [sys.executable, __file__, "--watch", transcript_path, session_id],
...
def cmd_watch(transcript_path: str, session_id: str):
    """Background watcher process."""
    try:
        watch(transcript_path, session_id)
    finally:
        _pid_path(session_id).unlink(missing_ok=True)
...
if __name__ == "__main__":
    ...
    if mode == "--start":
        ...
        cmd_stop()
    elif mode == "--watch" and len(sys.argv) == 4:
